...
const getAllChirpsByID = `-- name: GetAllChirpsByID :one
SELECT id, created_at, updated_at, body, user_id FROM chirps where id = $1 ORDER BY created_at ASC
`

func (q *Queries) GetAllChirpsByID(ctx context.Context, id uuid.UUID) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, getAllChirpsByID, id)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
...
func (q *Queries) GetAllChirpsByUser(ctx context.Context, userID uuid.NullUUID) ([]Chirp, error) {
